...
async def list_products(limit: int = 20, offset: int = 0) -> dict:
    ...
    return {"items": [{"id": p.id, "sku": p.sku, "name": p.name,
                       "price": float(p.price), "category": p.category.name}
                      for p in rows], "limit": limit, "offset": offset}


@app.get("/products/{pid}")
async def get_product(pid: int) -> dict:
    async with Session() as sess:
        p = (await sess.execute(
            select(Product).options(selectinload(Product.category))
...
async def list_products_n1(limit: int = 20) -> dict:
    ...
    async with Session() as sess:
        ids = (await sess.execute(
            select(Product.id).order_by(Product.id).limit(min(limit, 100))
        )).scalars().all()
        out = []
